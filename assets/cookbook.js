...
import {subscribe} from '../../assets/feed-client.js';
...
const stop = subscribe(document.body.dataset.api, (feed, error) => {
    status.textContent = error ? t('API unavailable') : '';
    ...
    [temperatureHumidity(data, zone), dailyRain(data.rain7d, zone), rainAccumulation(data.rain24h, zone),
        monthlyComparison(data.rainComparison, zone)].forEach((option, i) => {
            option.aria = {enabled: true, label: {description: targets[i].getAttribute('aria-label') + t('. Values are available in the data table.')}};
    ...
    for (const name of ['temperature24h', 'rain7d', 'rain24h', 'rainComparison']) {
        ...
        const states = {ready: '', stale: t('Update pending'), pending: t('Pending'), unavailable: t('No data')};
        result.textContent = [stamps, states[field?.status] || '',
            name === 'rainComparison' && !(field?.periods?.points?.length) ? t('No complete comparison years') : ''].filter(Boolean).join(' · ');
    }
